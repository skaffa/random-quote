...
def load_quotes():
    with open("quotes.pkl", "rb") as quotes_file:
        quotes = pickle.load(quotes_file)
    return quotes

def sentence_splitter(text):
    # Use a simple sentence splitting regex
...
def get_quote():
    ...
    # Concatenate all quotes into a single text
    text = ". ".join([quote['quote'] for quote in quotes])

    # Split the text into sentences
    sentences = sentence_splitter(text)

    # Create a text model
    model = markovify.NewlineText("\n".join(sentences), state_size=2)
...
def generate_random_gradient_with_quote():
    ...
    image.save(f'images/{filename}.png')  # Corrected the variable name here
